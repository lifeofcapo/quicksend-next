...
<section id="hero-section" className="pt-32 pb-20 px-4 bg-white dark:bg-gray-900 transition-colors">
  <div className="container mx-auto text-center">
    <h1 className="text-5xl md:text-7xl font-bold text-gray-900 dark:text-white mb-6">{t.title}</h1>
    <p className="text-xl text-gray-600 dark:text-gray-300 max-w-2xl mx-auto mb-8">{t.description}</p>
...
<section id="whyquicksend" className="py-20 px-4 bg-gray-50 dark:bg-gray-800 transition-colors">
  <div className="container mx-auto">
    ...
    <p className="text-xl text-gray-600 dark:text-gray-300 text-center max-w-3xl mx-auto mb-16">{t.whyDesc}</p>

    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
      {advantages.map((a, i) => {
        const Icon = a.icon;
        return (
          <div key={i} className="bg-white dark:bg-gray-700 p-8 rounded-xl shadow-lg hover:shadow-xl transition">
...
<section id="products" className="py-20 px-4 bg-white dark:bg-gray-900 transition-colors">
  <div className="container mx-auto">
    <h2 className="text-4xl font-bold text-center mb-16 dark:text-white">{t.productsTitle}</h2>

    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
      {products.map((p, i) => {
        const Icon = p.icon;
        return (
          <div key={i} className="bg-white dark:bg-gray-800 p-8 rounded-xl border border-gray-200 dark:border-gray-700 hover:border-blue-400 dark:hover:border-blue-400 hover:shadow-xl transition">
            <Icon className="w-12 h-12 text-blue-600 dark:text-blue-400 mb-4" />
    ...
    <div className="text-center mt-12">
      <button className="px-8 py-4 bg-blue-600 dark:bg-blue-700 text-white rounded-lg hover:bg-blue-700 dark:hover:bg-blue-600 transition text-lg font-semibold">{t.addToGmail}</button>
      <p className="text-gray-500 dark:text-gray-400 mt-4 text-sm">{t.chromeRequired}</p>
    </div>
  </div>
</section>
...
<section className="py-20 px-4 bg-gradient-to-r from-blue-600 to-purple-600 dark:from-blue-700 dark:to-purple-700 text-white transition-colors">
  <div className="container mx-auto">
    <div className="grid grid-cols-1 md:grid-cols-2 gap-12 text-center">
      <div>
        <p className="text-lg mb-2 opacity-90">{t.statsEmails}</p>
        <span className="text-5xl md:text-6xl font-bold">{emailCount.toLocaleString()}</span>
      </div>
      <div>
        <p className="text-lg mb-2 opacity-90">{t.statsCampaigns}</p>
...
<section id="pricing" className="py-20 px-4 bg-gray-50 dark:bg-gray-900 transition-colors">
  <div className="container mx-auto">
    ...
    <p className="text-xl text-gray-600 dark:text-gray-400 text-center mb-8">{t.pricingSub}</p>
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-6xl mx-auto">
      {plans.map((plan, idx) => {
        ...
        return (
          <div key={idx} className={`bg-white dark:bg-gray-800 rounded-xl p-8 transition-colors ${plan.popular ? 'ring-2 ring-blue-600 dark:ring-blue-400 shadow-xl scale-105' : 'border border-gray-200 dark:border-gray-700'}`}>
            {plan.popular && <span className="bg-blue-600 dark:bg-blue-500 text-white px-4 py-1 rounded-full text-sm font-semibold mb-4 inline-block">{t.popular}</span>}

            <Icon className="w-12 h-12 text-blue-600 dark:text-blue-400 mb-4" />
            <h3 className="text-2xl font-bold mb-4 dark:text-white">{pl(plan.name)}</h3>

            <div className="mb-6">
              {plan.price[language] === 0 ? (
                <span className="text-4xl font-bold dark:text-white">{t.free}</span>
              ) : (
                <>
            ...
            <Link href="/pricing" className="block w-full py-3 px-6 rounded-lg text-center font-semibold transition bg-blue-600 dark:bg-blue-700 text-white hover:bg-blue-700 dark:hover:bg-blue-600">
              {pl(plan.button)}
            </Link>
          </div>
        );
...
<section id="contact" className="py-20 px-4 bg-white dark:bg-gray-900 transition-colors">
  <div className="container mx-auto text-center max-w-3xl">
    ...
    <p className="text-gray-600 dark:text-gray-400 text-lg">{t.feedback}</p>
  </div>
</section>
